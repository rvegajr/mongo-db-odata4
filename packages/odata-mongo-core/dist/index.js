import { BadRequestError } from 'odata-mongo-contracts';
...
export class MongoQueryBuilder {
    async execute(query, collection, options) {
        ...
        const hasSearch = typeof query.search === 'string' && query.search.length > 0 && options?.search?.fields?.length;
        const hasApply = !!query.apply && Array.isArray(query.apply.groupByFields) && query.apply.groupByFields.length > 0 && Array.isArray(query.apply.aggregates) && query.apply.aggregates.length > 0;
        const hasCompute = !!query.compute;
        let docs;
        if ((hasExpand || hasSearch || hasApply || hasCompute) && typeof collection.aggregate === 'function') {
            const expandMap = options?.expandMap;
            ...
            if (hasApply) {
                ...
                const groupId = {};
                for (const f of apply.groupByFields)
                    groupId[f] = `$${f}`;
                const groupStage = { _id: groupId };
                for (const agg of apply.aggregates) {
                    const opMap = { sum: '$sum', avg: '$avg', min: '$min', max: '$max', count: '$sum' };
        ...
        return { value: docs, count, nextLink };
    }
}
function buildFilter(filter, options) {
    ...
    if (typeof filter === 'string') {
        try {
            ...
            if (options?.fieldMap)
                applyFieldMap(mongo, options.fieldMap);
...
function isFilterAllowed(node, allowed) {
    ...
    for (const key of Object.keys(node)) {
        ...
        if (val && typeof val === 'object') {
            if (!isFilterAllowed(val, allowed))
                return false;
        }
    }
    return true;
}
function applyFieldMap(node, fieldMap) {
...
function buildSort(orderBy) {
    ...
    for (const ob of orderBy)
        sort[ob.field] = ob.direction === 'desc' ? -1 : 1;
    return sort;
}
function sanitizeTop(top, maxTop, defaultPageSize) {
...
function sanitizeSkip(skip) {
    if (skip == null)
        return undefined;
    const n = Math.max(0, Number(skip) || 0);
    return n || undefined;
}
function buildNextLink(options, query, returned) {
    if (!query.top)
        return undefined;
    if (returned < (query.top || 0))
        return undefined;
    const base = options?.baseUrl || '';
    const parts = [];
    if (typeof query.top === 'number')
        parts.push(`$top=${encodeURIComponent(String(query.top))}`);
    const newSkip = (query.skip || 0) + (query.top || 0);
    parts.push(`$skip=${encodeURIComponent(String(newSkip))}`);
    if (query.select?.length)
        parts.push(`$select=${encodeURIComponent(query.select.join(','))}`);
    if (query.orderBy?.length)
        parts.push(`$orderby=${encodeURIComponent(query.orderBy.map(o => `${o.field} ${o.direction}`).join(','))}`);
    if (typeof query.count === 'boolean')
        parts.push(`$count=${encodeURIComponent(String(query.count))}`);
    if (typeof query.filter === 'string')
        parts.push(`$filter=${encodeURIComponent(query.filter)}`);
    if (query.expand?.length)
        parts.push(`$expand=${encodeURIComponent(query.expand.map(e => e.path).join(','))}`);
    const qs = parts.join('&');
