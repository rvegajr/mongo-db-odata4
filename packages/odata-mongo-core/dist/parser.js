export class ODataParserV4Minimal {
    parse(url) {
        ...
        for (const m of markers)
            if (f.includes(m))
                unimplementedFunctions.push(m);
        return { top, skip, select, orderBy, count, expand, filter: rawFilter, search: searchTerm, apply, unimplementedFunctions, ...(compute ? { compute } : {}) };
    }
}
function num(v) {
    if (v == null)
...
function parseApply(v) {
    ...
    return { groupByFields, aggregates: [{ source, op: op.toLowerCase(), as }] };
}
function parseCompute(v) {
    if (!v)
        return undefined;
    // minimal: <field> mul <num> as <alias>
    const m = /(\w+)\s+mul\s+(\d+(?:\.\d+)?)\s+as\s+(\w+)/i.exec(v);
    if (m)
        return { expr: `${m[1]}*${m[2]}`, as: m[3] };
    const c = /concat\((\w+),\'([^']*)\'\)\s+as\s+(\w+)/i.exec(v);
    if (c)
        return { expr: `concat:${c[1]}:${c[2]}`, as: c[3] };
    const tl = /tolower\((\w+)\)\s+as\s+(\w+)/i.exec(v);
    if (tl)
        return { expr: `tolower:${tl[1]}`, as: tl[2] };
    const tu = /toupper\((\w+)\)\s+as\s+(\w+)/i.exec(v);
    if (tu)
        return { expr: `toupper:${tu[1]}`, as: tu[2] };
    const ss = /substring\((\w+),(\d+),(\d+)\)\s+as\s+(\w+)/i.exec(v);
    if (ss)
        return { expr: `substring:${ss[1]}:${ss[2]}:${ss[3]}`, as: ss[4] };
    const rd = /round\((\w+)\)\s+as\s+(\w+)/i.exec(v);
    if (rd)
        return { expr: `round:${rd[1]}`, as: rd[2] };
    const fl = /floor\((\w+)\)\s+as\s+(\w+)/i.exec(v);
    if (fl)
        return { expr: `floor:${fl[1]}`, as: fl[2] };
    const ce = /ceiling\((\w+)\)\s+as\s+(\w+)/i.exec(v);
    if (ce)
        return { expr: `ceiling:${ce[1]}`, as: ce[2] };
    const nest = /tolower\(substring\((\w+),(\d+),(\d+)\)\)\s+as\s+(\w+)/i.exec(v);
    if (nest)
        return { expr: `tolower:substring:${nest[1]}:${nest[2]}:${nest[3]}`, as: nest[4] };
    const len = /length\((\w+)\)\s+as\s+(\w+)/i.exec(v);
    if (len)
        return { expr: `length:${len[1]}`, as: len[2] };
    const tr = /trim\((\w+)\)\s+as\s+(\w+)/i.exec(v);
    if (tr)
        return { expr: `trim:${tr[1]}`, as: tr[2] };
    const io = /indexof\((\w+),\'([^']*)\'\)\s+as\s+(\w+)/i.exec(v);
    if (io)
        return { expr: `indexof:${io[1]}:${io[2]}`, as: io[3] };
    const rp = /replace\((\w+),\'([^']*)\',\'([^']*)\'\)\s+as\s+(\w+)/i.exec(v);
    if (rp)
        return { expr: `replace:${rp[1]}:${rp[2]}:${rp[3]}`, as: rp[4] };
    const rpa = /replaceall\((\w+),\'([^']*)\',\'([^']*)\'\)\s+as\s+(\w+)/i.exec(v);
    if (rpa)
        return { expr: `replaceall:${rpa[1]}:${rpa[2]}:${rpa[3]}`, as: rpa[4] };
    const sw = /startswith\((\w+),\'([^']*)\'\)\s+as\s+(\w+)/i.exec(v);
    if (sw)
        return { expr: `startswith:${sw[1]}:${sw[2]}`, as: sw[3] };
    const ew = /endswith\((\w+),\'([^']*)\'\)\s+as\s+(\w+)/i.exec(v);
    if (ew)
        return { expr: `endswith:${ew[1]}:${ew[2]}`, as: ew[3] };
    const yr = /year\((\w+)\)\s+as\s+(\w+)/i.exec(v);
    if (yr)
        return { expr: `year:${yr[1]}`, as: yr[2] };
    const mo = /month\((\w+)\)\s+as\s+(\w+)/i.exec(v);
    if (mo)
        return { expr: `month:${mo[1]}`, as: mo[2] };
    const dy = /day\((\w+)\)\s+as\s+(\w+)/i.exec(v);
    if (dy)
        return { expr: `day:${dy[1]}`, as: dy[2] };
    return undefined;
}
