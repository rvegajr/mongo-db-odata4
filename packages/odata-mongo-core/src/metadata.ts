import { IMetadataProvider, ODataSchemaMetadata, JsonSchemaLike, ODataPropertyMetadata, ODataEdmType } from 'odata-mongo-contracts';

// Minimal, static XML generator for OData v4 CSDL, single entity set
export class MinimalMetadataProvider implements IMetadataProvider {
...
export function inferPropertiesFromJsonSchema(schema: JsonSchemaLike | undefined): ODataPropertyMetadata[] | undefined {
  if (!schema || !schema.properties) return undefined;
  const required = new Set(schema.required ?? []);
  const props: ODataPropertyMetadata[] = [];
...
function jsonTypeToEdm(type: string | string[] | undefined, format?: string): ODataEdmType | undefined {
  const t = Array.isArray(type) ? type[0] : type;
  switch (t) {
    case 'string':
      if (format === 'date-time') return 'Edm.DateTimeOffset';
      if (format === 'uuid') return 'Edm.Guid';
      return 'Edm.String';
    case 'integer':
      return 'Edm.Int32';
    case 'number':
