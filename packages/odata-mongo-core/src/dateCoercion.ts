import type { DateCoercionOptions } from 'odata-mongo-contracts';
...
function tryParseWithKnownFormats(s: string, formats: string[]): Date | null {
...
export function coerceDatesInDocuments(
  docs: any[],
  options: DateCoercionOptions & { database?: string; collection?: string }
): any[] {
    ...
    for (const f of fields) {
      ...
      try {
        ...
        if (v instanceof Date) {
          ...
          if (epochMode === 'ms' || (epochMode === 'auto' && v > 1e10)) parsed = new Date(v);
          else if (epochMode === 's' || (epochMode === 'auto' && v <= 1e10)) parsed = new Date(v * 1000);
        } else if (typeof v === 'string') {
          parsed = tryParseWithKnownFormats(v, formats);
        }
        if (parsed && !Number.isNaN(parsed.getTime())) doc[f] = parsed;
      } catch (err: any) {
        if (options.onError) {
          options.onError({
            database,
