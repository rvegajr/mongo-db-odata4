...
export interface ODataQuery {
  top?: number;
  skip?: number;
  filter?: unknown; // AST to be refined in Phase 2
  orderBy?: ODataOrderByField[];
  select?: string[];
  expand?: Array<{ path: string }>;
  count?: boolean;
  search?: string;
  apply?: {
    groupByFields?: string[];
    aggregates?: Array<{ source: string; op: 'sum'|'avg'|'min'|'max'|'count'; as: string }>;
  };
...
export type ExpandMapEntry = { from: string; localField: string; foreignField: string; as?: string; single?: boolean };
export interface ExecuteOptions {
  ...
  fieldMap?: Record<string, string>; // apiName -> dbName (e.g., id -> _id)
  hooks?: {
    onParsed?: (query: ODataQuery) => void;
    onExecuted?: (info: { query: ODataQuery; durationMs: number; resultCount: number }) => void;
...
export interface ODataSchemaMetadata {
  ...
  container: string; // e.g., "Container"
